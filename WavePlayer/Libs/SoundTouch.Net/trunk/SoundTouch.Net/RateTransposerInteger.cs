    ...
    internal sealed class RateTransposerInteger : RateTransposer<TSampleType> 
    {
        ...
        protected override int TransposeMono(ArrayPtr<TSampleType> dst, ArrayPtr<TSampleType> src, int numSamples)
        {
            ...
            while (true)
            {
                ...
                dst[i] = (TSampleType) (Volume* (temp / SCALE));
        ...
        protected override int TransposeStereo(ArrayPtr<TSampleType> dst, ArrayPtr<TSampleType> src, int numSamples)
        {
            ...
            while (_slopeCount <= SCALE) 
            {
                ...
                {
                    String msg = string.Format("access dst {0}, dst.len={1} src[0]~src[1],src.len={2}"+Environment.NewLine, 2 * i, dst.GetLength(), src.GetLength());
                    msg += string.Format("access dst {0}, dst.len={1} src[1],src.len={2}" + Environment.NewLine, 2 * i + 1, dst.GetLength(), src.GetLength());
                    Console.WriteLine(msg);
                    Logger.Log.WriteLine(msg);
                    Logger.Log.Flush();
                }
                i++;
                _slopeCount += _rate;
            ...
            while (true)
            {
                ...
                try
                {
                    if (2 * i + 1 < dst.GetLength() && srcPos + 3 < src.GetLength())
                    {
                        temp = src[srcPos] * vol1 + _slopeCount * src[srcPos + 2];
                        dst[2 * i] = (TSampleType)(Volume*(temp / SCALE));
                        temp = src[srcPos + 1] * vol1 + _slopeCount * src[srcPos + 3];
                        dst[2 * i + 1] = (TSampleType)(Volume*(temp / SCALE));
                    }
                }
                catch (Exception ee)
                {
                    String msg = ee.ToString()+ string.Format("access dst {0}~{1} dst.len={1} " + Environment.NewLine,2*i,2*i+1,dst.GetLength() );
                    msg += string.Format("access src {0}~{1} src.len={2} " + Environment.NewLine, srcPos,srcPos+3, src.GetLength());
                    Console.WriteLine(msg);
                    goto end;
                }
            ...
            // Store the last sample for the next round
            if (2 * numSamples - 1 < src.GetLength())
            {
