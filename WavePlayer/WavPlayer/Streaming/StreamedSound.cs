...
namespace WavePlayer.WavPlayer.Streaming
{
    ...
    {
        private List<Interfaces.IEffectOperator> mEffects = new List<Interfaces.IEffectOperator>();
        public List<Interfaces.IEffectOperator> Effects
        {
            get
            {
                return mEffects;
        ...
        public void OnBufferNotification_V1(object sender, BufferNotificationEventArgs e)
        {
            ...
            try
            {
                ...
                if (Effects.Count > 0)
                {
                    Interfaces.EffectEventArgs EffectArg = new Interfaces.EffectEventArgs();
                    EffectArg.NewSoundByte = null;
                    EffectArg.Stream = Stream;
                    EffectArg.NumBytesRequired = e.NumBytesRequired;
                    for (int i = 0; i < Effects.Count; ++i)
                    {
                        Interfaces.IEffectOperator effect = Effects[i];
                        if (effect != null)
                        {
                            effect.Handle(EffectArg);
                        }
                    }
                    
                    e.NewSoundByte = EffectArg.NewSoundByte;
