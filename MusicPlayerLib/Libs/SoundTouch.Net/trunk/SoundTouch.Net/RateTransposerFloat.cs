        ...
        protected override int TransposeStereo(ArrayPtr<TSampleType> dst, ArrayPtr<TSampleType> src, int numSamples)
        {
            ...
            if (numSamples > 1)
            {
                while (true)
                {
                    ...
                    dst[2 * i] = (TSampleType) Volume* ((1.0f - _slopeCount) * src[srcPos] + _slopeCount * src[srcPos + 2]);
                    dst[2 * i + 1] = (TSampleType)Volume * ((1.0f - _slopeCount) * src[srcPos + 1] + _slopeCount * src[srcPos + 3]);
